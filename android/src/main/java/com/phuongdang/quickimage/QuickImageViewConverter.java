...
import static com.phuongdang.quickimage.QuickImageCacheControl.CACHE_ONLY;
import static com.phuongdang.quickimage.QuickImageCacheControl.IMMUTABLE;
import static com.phuongdang.quickimage.QuickImageCacheControl.WEB;
...
public class QuickImageViewConverter {
  ...
  private static final Map<String, Priority> QUICK_IMAGE_PRIORITY_MAP =
  ...
  private static final Map<String, ImageView.ScaleType> QUICK_IMAGE_RESIZE_MODE_MAP =
  ...
  private static QuickImageCacheControl getCacheControl(ReadableMap source) {
    return getValueFromSource("cache", "immutable", QUICK_IMAGE_CACHE_CONTROL_MAP, source);
  }

  private static Priority getPriority(ReadableMap source) {
    return getValueFromSource("priority", "normal", QUICK_IMAGE_PRIORITY_MAP, source);
  }

  static @Nullable QuickImageSource getImageSource(Context context, @Nullable ReadableMap source){
    return source == null
      ? null
      : new QuickImageSource(context, source.getString("uri"), getHeaders(source));
  }

  static RequestOptions getOptions(Context context, QuickImageSource imageSource, ReadableMap source) {
    // Get priority.
    final Priority priority = QuickImageViewConverter.getPriority(source);
    // Get cache control method.
    final QuickImageCacheControl cacheControl = QuickImageViewConverter.getCacheControl(source);
  ...
  static ScaleType getScaleType(String propValue) {
    return getValue("resizeMode", "cover", QUICK_IMAGE_RESIZE_MODE_MAP, propValue);
  }
}
